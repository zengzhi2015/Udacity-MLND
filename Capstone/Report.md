...
The proposed method is mainly based on convolutional neural networks (CNN). CNNs are a category of neural networks that have proven very effective in areas such as image recognition and classification. CNNs have been successful in identifying faces, objects, and traffic signs apart from powering vision in robots and self-driving cars. One could see this [webpage](https://ujjwalkarn.me/2016/08/11/intuitive-explanation-convnets/) for a quick understanding of this technique [12]. 
...
It should be noted that the each of these models can be separated into two parts. The first part is called the feature extraction network which is composed of several connected forward CNNs. The feature extraction network can be shared between all three models. This is because the input for the three networks is very similar. The only difference between these models lies in their top layers. Usually, the top layers are densely connected neural networks. In this project, all top layers of three networks have exactly the same structure except for the final layer. Details regarding these networks are as follows.
